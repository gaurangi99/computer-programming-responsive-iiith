window.view = {
	...
	showSquareInputs: function() {
		this.hideQuestionBlock('questionRightTriangle', 'imageRightTriangle')
	...
	validateRightTriangleInputs: function() {
		...
		if ( a !== '2')
			alert('Incorrect value of input variables(arguments). Calculating the area of a Right Triangle requires the length of the two perpendicular sides of the Right Triangle. Try again.')
		else if ( b !== 'float' )
			...
			alert('Incorrect datatype for return type. The value of the area of a Right Triangle need not be an integer. Try again.')
		else if ( d !== '(1/2)*a*b' )
			alert('Incorrect formula for calculating the area of a Right Triangle. Try again.')
		else
	...
	displayFunctionForRightTriangle: function() {
		document.getElementById('functionRightTriangle').innerHTML += 'float area_rightTri (float a,float b)<br>{<br> &emsp; float area = (1/2)*a*b;<br> &emsp; return area;<br>}'
	},
	...
	area_rightTri: function( param, functionCall ) {
	...
	resetToInitialState: function() {
		...
		document.getElementById('imageRightTriangle').className = 'imageRightTriangle'
		...
		document.getElementById('tickRightTriangle').className = 'tick hide'
	...
	calculateArea: function() {
		...
		for ( i = 0 ; i < elements.length ; i ++ ) {
			...
			else if ( parameter.length === 2 && functionCall === 'area_rightTri()' && isNaN(parameter[0]) === false && isNaN(parameter[1]) === false )
				this.area_rightTri(parameter, parsedValue)
			else
		 	alert( 'Incorrect function call at line ' + String( i + 1 ) )
		}	
